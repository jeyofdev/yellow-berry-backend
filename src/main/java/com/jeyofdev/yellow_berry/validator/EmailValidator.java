...
public class EmailValidator implements ConstraintValidator<ValidEmail, String> {
    ...
    public boolean isValid(String value, ConstraintValidatorContext context) {
        if (value == null || value.isEmpty()) {
            addConstraintViolation(context, ErrorMessage.EMAIL_REQUIRED);
            return false;
        }

        if (value.length() > 100) {
            addConstraintViolation(context, ErrorMessage.EMAIL_LENGTH);
            return false;
        }

        if (!value.matches(Regex.EMAIL_PATTERN)) {
            addConstraintViolation(context, ErrorMessage.EMAIL_FORMAT);
            return false;
        }
